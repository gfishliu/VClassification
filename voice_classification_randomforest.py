...
def dRPCA(data, COMPONENT_NUM=100):
    ...
    storeModel(data_pca, os.path.join(data_dir, 'Result_sklearn_rf.pcaData'))
    return data_pca
...
def printAccuracy(y_test ,y_predict):
    ...
    for i in range(len(zeroLable)):
        if list(zeroLable)[i] == 0:
            rightCount += 1
    print('Accuracy : %f' %  (float(rightCount) / len(zeroLable)))
    #print(metrics.accuracy_score(y_test, y_predict))
    print(metrics.classification_report(y_test, y_predict))
    CMatrix = metrics.confusion_matrix(y_test, y_predict)
    print(CMatrix)
    from voice_utils import plot_confusion_matrix
    plot_confusion_matrix(CMatrix, title="RF Confusion Matrix", classes=['Anger', 'Disgust', 'Fear', 'Happy', 'Sad', 'Surprise'])
...
def saveResult(result, csvName):
    ...
    with open(os.path.join(data_dir, 'Result_sklearn_RF.csv'), 'w') as fw:
...
def trainRF():
    ...
    # train_test_split(被划分的样本特征数据， 被划分的样本标签数据， 样本中用于测试的比例， 随机数种子)
    #X_train, X_test, y_train, y_test = train_test_split(
    #    data[0:len(train_data)], train_label, test_size=0.01, random_state=34)

    rfClf = trainModel(train_feature, train_label)
    #rfClf = trainModel(train_feature[6:], train_label[6:])
    #print(train_feature[0], train_label[0])

    # 保存结果
    #storeModel(data[len(train_data):], os.path.join(data_dir, 'Result_sklearn_rf.pcaPreData'))
    storeModel(feature[len(train_data):], os.path.join(data_dir, 'Result_sklearn_rf.pcaPreData'))
    storeModel(rfClf, os.path.join(data_dir, 'Result_sklearn_rf.model'))
...
def preRF():
    startTime = time.time()
    # 加载模型和数据
    clf=getModel(os.path.join(data_dir, 'Result_sklearn_rf.model'))
    pcaPreData = getModel(os.path.join(data_dir, 'Result_sklearn_rf.pcaPreData'))

    # 结果预测
    result = clf.predict(pcaPreData)

    # 结果的输出
    saveResult(result, os.path.join(data_dir, 'Result_sklearn_rf.csv'))
    printAccuracy(pd.read_csv(os.path.join(data_dir, 'test.csv'))['labels'], result)
    print("finish!")
    stopTime = time.time()
